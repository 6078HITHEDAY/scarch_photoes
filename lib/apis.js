...
export async function fetchRealPerson(type = 'cos') {
  try {
    ...
    if (json.code === 200 && json.url) {
      return { success: true, data: { imageUrl: json.url, type } }
    }
...
export async function fetchLandscape() {
  ...
  try {
    ...
    const json = await res.json()
    if (json.code === 200 && json.url) {
      return { success: true, data: { imageUrl: json.url, source: 'tomys' } }
    }
  } catch (_) { /* 降级 */ }
  ...
  try {
    ...
    return { success: false, error: `风景 API 请求失败: ${e.message}` }
  }
