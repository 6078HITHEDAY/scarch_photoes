...
/**
 * 美女图片 (pic.ltywl.top → TomyJan 降级)
 */
export async function fetchMeinv() {
  try {
    const url = config.endpoints.mnPic
  ...
  // 降级: TomyJan
  const tomyResult = await fetchRealPerson('cos')
  if (tomyResult.success) {
    tomyResult.data.source = 'tomys'
    return tomyResult
  }
